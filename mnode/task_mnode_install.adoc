...
이 절차를 따르기 전에 해당 기능을 사용할 것인지 및 사용할 것인지 여부를 이해해야 link:../concepts/concept_solidfire_concepts_volumes.html#persistent-volumes["영구 볼륨"]합니다. 영구 볼륨은 선택 사항이지만 가상 머신(VM)이 손실된 경우 관리 노드 구성 데이터 복구에 권장됩니다.

.단계
. <<ISO 또는 OVA를 다운로드하고 VM을 배포합니다>>
. <<create_mnode_admin,관리 노드 admin을 생성하고 네트워크를 구성합니다>>
. <<시간 동기화를 구성합니다>>
. <<관리 노드를 설정합니다>>
. <<컨트롤러 자산을 구성합니다>>
...
. NetApp 지원 사이트의 페이지에서 설치에 필요한 OVA 또는 ISO를 link:https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["Element 소프트웨어"^]다운로드합니다.
+
.. 최신 릴리스 다운로드 * 를 선택하고 EULA에 동의합니다.
.. 다운로드할 관리 노드 이미지를 선택합니다.


. OVA를 다운로드한 경우 다음 단계를 따르십시오.
+
.. OVA를 배포합니다.
...
NOTE: 스토리지 트래픽을 격리하기 위해 추가 NIC가 필요한 경우 다른 NIC 구성 지침을 참조하십시오link:task_mnode_install_add_storage_NIC.html["스토리지 NIC(Network Interface Controller) 구성"].
...
NOTE: 요소 12.3.1부터 하위 단계(a)에서 (e)까지 자동으로 수행됩니다. 관리 노드 12.3.1의 경우 로 <<substep_f_install_config_time_sync,하위 단계 (f)>> 이동하여 시간 동기화 구성을 완료합니다.
...
----
. NTP 구성 파일을 `/etc/ntp.conf`편집합니다.
+
.. 각 서버 앞에(`server 0.gentoo.pool.ntp.org` 를 추가하여 기본 서버에 대한 주석 처리) `#`.
.. 추가할 각 기본 시간 서버에 대해 새 줄을 추가합니다. 기본 시간 서버는 에서 사용할 스토리지 클러스터에서 사용되는 NTP 서버와 같아야 link:task_mnode_install.html#set-up-the-management-node["나중에"]합니다.
+
[listing]
----
vi /etc/ntp.conf
...
----
.. vSphere에서 `Synchronize guest time with host` VM 옵션에서 확인란이 선택 취소되어 있는지 확인합니다.
+
...
NOTE: 시간 동기화 구성을 완료한 후에는 NTP를 편집하지 마십시오. 관리 노드에서 를 실행할 때 NTP에 영향을 주기 때문입니다.link:task_mnode_install.html#set-up-the-management-node["설정 명령"]
...
*** * -- storage_mvip(-SM) [MVIP 주소] *: Element 소프트웨어를 실행하는 스토리지 클러스터의 관리 가상 IP 주소(MVIP)입니다. 에서 사용한 것과 동일한 스토리지 클러스터를 사용하여 관리 노드를 link:task_mnode_install.html#configure-time-sync["NTP 서버 구성"]구성합니다.
*** * - -storage_username(-su) [username] *: 매개 변수로 지정한 클러스터의 스토리지 클러스터 관리자 사용자 `--storage_mvip` 이름입니다.
...
*** * -- PERTERNAL_VOLUES_ACCOUNT(-PVA) [ACCOUNT_NAME] *: 이 true로 설정되어 있으면 `--use_persistent_volumes` 이 매개변수를 사용하고 영구 볼륨에 사용할 스토리지 계정 이름을 입력합니다.
+
...
*** * -- proxy_hostname_or_ip(-pi) [host] *: 프록시 호스트 이름 또는 IP 프록시를 사용하려면 이 옵션을 사용해야 합니다. 이 옵션을 지정하면 입력을 요청하는 메시지가 `--proxy_port` 표시됩니다.
*** * -- proxy_username(-pu) [username] *: 프록시 사용자 이름입니다. 이 매개 변수는 선택 사항입니다.
*** * -- proxy_password(-pp) [password] *: 프록시 암호입니다. 이 매개 변수는 선택 사항입니다.
*** * -- proxy_port(-PQ) [port, default:0] *: 프록시 포트. 이 옵션을 지정하면 프록시 호스트 이름 또는 IP를 (`--proxy_hostname_or_ip`입력하라는 메시지가 표시됩니다.
...
.. 스토리지 MVIP로 이동하여 로그인합니다. 이렇게 하면 다음 단계에서 인증서가 수락됩니다.
...
.. 체험하기 * 를 선택합니다.
.. Execute * 를 선택합니다.
.. 코드 200 응답 본문에서 나중에 사용할 수 있도록 설치를 위해 를 복사하여 `id` 저장합니다.
+
...
NOTE: 컨트롤러 하위 자산을 추가하려면 vCenter에서 새로운 NetApp HCC 역할을 생성해야 합니다. 이러한 새로운 NetApp HCC 역할은 관리 노드 서비스 뷰를 NetApp 전용 자산으로 제한합니다. 을 link:task_mnode_create_netapp_hcc_role_vcenter.html["vCenter에서 NetApp HCC 역할을 생성합니다"]참조하십시오.

.. 체험하기 * 를 선택합니다.
.. 클립보드에 복사한 상위 기본 자산 ID를 * asset_id * 필드에 입력합니다.
.. 유형 및 vCenter 자격 증명을 사용하여 필요한 페이로드 값을 `vCenter` 입력합니다.
.. Execute * 를 선택합니다.
